...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      const tracesResponse = await fetch(`${workerUrl}/traces/recent?limit=50&t=${Date.now()}`, {
        headers: { 
          'Content-Type': 'application/json',
          'Cache-Control': 'no-cache, no-store, must-revalidate',
          'Pragma': 'no-cache'
        },
        signal: typeof AbortSignal !== 'undefined' && AbortSignal.timeout ? AbortSignal.timeout(5000) : undefined, // 5 second timeout
      });
      ...
      if (tracesResponse.ok) {
        ...
        redisStats.total_logs = traces.reduce((sum: number, trace: any) => sum + (trace.logCount || 0), 0);
          ...
          if (trace.systems) {
            trace.systems.forEach((system: string) => {
              if (system in systemCounts) {
                systemCounts[system as keyof typeof systemCounts] += trace.logCount || 1;
              }
            });
          }
        ...
        redisStats.oldest_log_hours = Math.floor((Date.now() - oldestTimestamp) / (1000 * 60 * 60));
      }
    } catch (tracesError) {
      // eslint-disable-next-line no-console
      console.warn('Failed to fetch detailed Redis stats, using basic health data:', tracesError);
    ...
    const transformedData = {
      ...
      ttl_info: {
        ...
        expires_in_hours: Math.max(0, Math.floor(
          (healthData.components?.rate_limiter?.window_remaining_ms || 3600000) / (1000 * 60 * 60)
        )),
      }
    };
