...
export class RateLimiterDO {
  ...
  private async handleStatus(): Promise<Response> {
    ...
    return new Response(JSON.stringify({
      ...
      window_remaining_ms: Math.max(0, 
        RATE_LIMIT_CONFIG.window_ms - (Date.now() - this.rateLimitState!.window_start)
      ),
    }), {
      status: 200,
      headers: { 'content-type': 'application/json' }
  ...
  private async initializeState(): Promise<void> {
    if (!this.rateLimitState) {
      this.rateLimitState = await this.state.storage.get('rateLimitState') || {
        global_current: 0,
        system_current: {
          browser: 0,
          convex: 0,
