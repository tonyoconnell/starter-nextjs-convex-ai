...
describe('LogProcessor', () => {
  describe('validateRequest', () => {
    ...
    it('should reject invalid system values', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.error).toBe('system must be one of: browser, convex, worker, manual');
    });
  ...
  describe('redactSensitiveData', () => {
    it('should redact access tokens', () => {
      const sensitiveText = 'User data: {"access_token": "secret-token-123", "user": "john"}';
    ...
    it('should redact client secrets', () => {
      const sensitiveText = 'Config: {"client_secret": "super-secret", "app_id": "123"}';
      const redacted = LogProcessor.redactSensitiveData(sensitiveText);
    ...
    it('should redact refresh tokens', () => {
      const sensitiveText = 'Tokens: {"refresh_token": "refresh-xyz", "expires_in": 3600}';
      const redacted = LogProcessor.redactSensitiveData(sensitiveText);
    ...
    it('should redact generic tokens', () => {
      const sensitiveText = 'Auth: {"token": "generic-token", "type": "bearer"}';
      const redacted = LogProcessor.redactSensitiveData(sensitiveText);
    ...
    it('should redact passwords', () => {
      const sensitiveText = 'Login: {"username": "user", "password": "secret123"}';
      const redacted = LogProcessor.redactSensitiveData(sensitiveText);
    ...
    it('should redact generic secrets', () => {
      const sensitiveText = 'Config: {"secret": "my-secret", "public": "safe-data"}';
      const redacted = LogProcessor.redactSensitiveData(sensitiveText);
    ...
    it('should redact API keys', () => {
      const sensitiveText = 'API config: {"api_key": "key-123", "endpoint": "https://api.example.com"}';
      const redacted = LogProcessor.redactSensitiveData(sensitiveText);
    ...
    it('should redact Bearer tokens', () => {
      const sensitiveText = 'Authorization: bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.test.signature';
      const redacted = LogProcessor.redactSensitiveData(sensitiveText);
      
      expect(redacted).toContain('[REDACTED]');
      expect(redacted).not.toContain('eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.test.signature');
    });
    ...
    it('should preserve non-sensitive data', () => {
      const nonSensitiveText = 'User profile: {"name": "John", "email": "john@example.com", "age": 30}';
  ...
  describe('detectSystemFromHeaders', () => {
    it('should detect browser from localhost origin', () => {
      const headers = TestUtils.mockHeaders({ origin: 'http://localhost:3000' });
    ...
    it('should detect browser from 127.0.0.1 origin', () => {
      const headers = TestUtils.mockHeaders({ origin: 'http://127.0.0.1:3000' });
      const system = LogProcessor.detectSystemFromHeaders(headers);
  ...
  describe('processLogRequest', () => {
    it('should process valid log request successfully', () => {
      ...
      expect(result.processedEntry.timestamp).toBeGreaterThan(Date.now() - 1000);
    });
    ...
    it('should redact sensitive data in messages', () => {
      const request: WorkerLogRequest = {
        trace_id: 'test-trace-123',
        message: 'Login successful: {"access_token": "secret-123", "user": "john"}',
        level: 'info',
      };
    ...
    it('should redact sensitive data in context', () => {
      ...
      expect(result.shouldProcess).toBe(true);
      expect(JSON.stringify(result.processedEntry.context)).toContain('[REDACTED]');
      expect(JSON.stringify(result.processedEntry.context)).not.toContain('key-123');
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should handle special characters in messages', () => {
      const specialMessage = 'Test with emojis 🚀 and unicode ñáéíóú and symbols @#$%^&*()';
      const request: WorkerLogRequest = {
        trace_id: 'test-trace-123',
